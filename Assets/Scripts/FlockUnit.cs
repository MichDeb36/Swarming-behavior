...
public class FlockUnit : MonoBehaviour
{
	...
	public void ActiveUnit(bool active)
	{
		if (active)
	...
	private void FindNeighbours()
	{
		...
		var activatedUnits = assignedFlock.activatedUnits;
		for (int i = 0; i < activatedUnits.Count; i++)
		{
			var currentUnit = activatedUnits[i];
			if (currentUnit != this)
			{
	...
	{
		float valueMultiplierBoundsDistance = 0.9f;
		var offsetToCenter = assignedFlock.transform.position - myTransform.position;
		bool isNearCenter = (offsetToCenter.magnitude >= assignedFlock.boundsDistance * valueMultiplierBoundsDistance);
